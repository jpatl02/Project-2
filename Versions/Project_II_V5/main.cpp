 ...
 * Created on February 10th, 2023, 12:14 PM
...
int main(int argc, char** argv) {
    ...
    in.open("input6.dat",ios::in);               //Input and output files
    out.open("LCRoutput6.dat",ios::out);
    ...
    indx=binSrch(count, p1array, max);
    if (indx!=-1){
        cout<<max<<" was found at index = "<<indx<<endl;
    }
...
int binSrch(int count, int p1array[], int max){
    int first=0,
        last=count-1;
    while (first<=last){
        int middle=(first+last)/2;
        if (p1array[middle]==max){
            return middle;
        }else if (p1array[middle]>max){
            last=middle-1;
        }else{
            first=middle+1;
        }
