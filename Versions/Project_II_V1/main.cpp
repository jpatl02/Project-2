 ...
 * Created on February 6th, 2023, 3:43 PM
...
int main(int argc, char** argv) {
    ...
    in.open("input2.dat",ios::in);               //Input and output files
    out.open("LCRoutput2.dat",ios::out);
...
float average(float p1avg, int count){
    p1avg/=count;                       //Calculate player 1 average
    return static_cast<int>(p1avg);
}

float average(float p1avg, unsigned char count){
    p1avg/=count;                       //Calculate player 1 average
    return static_cast<int>(p1avg);
}
